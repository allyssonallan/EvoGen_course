...
# ANSWER: the number of sites is simply the sum of all entries in the SFS
...
# this is the unfolded spcetrum BUT it has been generated using a reference allele to polarise the spectrum; this means that the polarisation is arbitrary and we need to use the folded spectrum instead
...
for (i in 1:nr_simul) # run this "for loop" nr_simul times
{
	...
	ms.command <- paste("Software/msdir/ms 50", nr_snps, "-s 1 -I 2 36 14 -n 1 1 -n 2 6.8 -en 0.02269 1 0.07353 -en 0.05281 2 0.2941 -em 0.06459 1 2 4.896 -em 0.1392 1 2 0 -en 0.1392 1 0.2941 -ej", tdiv_random_coal, "2 1 -en 0.3924 1 1.809 > ms.txt")
	# run ms
	system(ms.command)
...
cat("", file="ms.txt")
ms.command <- paste("Software/msdir/ms 50", nr_snps, "-s 1 -I 2 36 14 -n 1 1 -n 2 6.8 -en 0.02269 1 0.07353 -en 0.05281 2 0.2941 -em 0.06459 1 2 4.896 -em 0.1392 1 2 0 -en 0.1392 1 0.2941 -ej", tdiv.est, "2 1 -en 0.3924 1 1.809 > ms.txt")
system(ms.command)
